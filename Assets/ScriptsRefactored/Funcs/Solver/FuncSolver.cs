...
namespace FuncSpace
{
    ...
    {
        public float Solve(float x, float y, float z, Bytecode code, float[] memory)
        {
            SetPositionToMemory(x, y, z, memory);
            SolveOperations(code, memory);
            return GetFinalResult(code, memory);
        }

        private void SetPositionToMemory(float x, float y, float z, float[] memory)
        {
            memory[0] = 0;
            memory[1] = x;
            memory[2] = y;
