...
namespace RenderingSpace
{
    ...
    {
        public GPURenderer(RawImage image, int level = 0, QuadRenderer parent = null) : base(image, level, parent) { }
        ...
        {
            Vector3 right = (viewFacade.NearTopRight - viewFacade.NearTopLeft).normalized;
            Vector3 up = (viewFacade.NearTopLeft - viewFacade.NearBottomLeft).normalized;
            float nearSize = Vector3.Distance(viewFacade.NearTopLeft, viewFacade.NearTopRight);
            float farSize = Vector3.Distance(viewFacade.FarTopLeft, viewFacade.FarTopRight);
