...
namespace RenderingSpace
{
    public class RendererFactory : IRendererFactory
    {
        ...
        public IRenderer CreateCPURenderer(int level = 0, QuadRenderer parent = null)
        {
            var image = CreateTexObject(level, parent);
            var renderer = new CPURenderer(image, level, parent);
            ...
            if (level < RenderConfig.cpuSetting.Length - 1)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    children[i] = (CPURenderer)CreateCPURenderer(level + 1, renderer);
        ...
        public IRenderer CreateGPURenderer(int level = 0, QuadRenderer parent = null)
        {
            var image = CreateTexObject(level, parent);
            var renderer = new GPURenderer(image, level, parent);
            ...
            if (level < RenderConfig.gpuSetting.Length - 1)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    children[i] = (GPURenderer)CreateGPURenderer(level + 1, renderer);
                    children[i].ChildIndex = i;
                }
                renderer.SetChildren(children);
            }
